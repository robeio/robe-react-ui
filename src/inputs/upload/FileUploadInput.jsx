...
import { Generator, Objects, Maps } from "robe-react-commons";
...
export default class FileUploadInput extends ValidationComponent {
    ...
    createToolbar(position){
        ...
        return (
            <div className={`rb-upload-toolbar rb-radius-${this.props.toolbarPosition}`}>
                <Checkbox className="pull-left" checked readOnly>
                    Select All
                </Checkbox>
                <Glyphicon className="pull-right" glyph="cloud-download"/>
                <Glyphicon className="pull-right" glyph="cloud-upload"/>
    ...
    renderItems(files){
        ...
        Maps.forEach(files, (file, key) => {
            let deleteIcon =  <FaIcon code="fa-minus-square-o pull-right"  size="fa-sm" onClick={this.deleteFile.bind(undefined, file)} />
            let uploadIcon = !file.isUploaded ? (
                    <FaIcon code="fa-upload pull-left"  size="fa-sm" onClick={this.uploadFile.bind(undefined, file)} />
                ): null;
            elements.push(
                <ThumbnailItem key={file.key} >
                    <div className={`rb-thumbnail-toolbar pull-top`}>
                        {uploadIcon}
                        {deleteIcon}
                    </div>
