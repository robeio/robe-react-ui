...
describe("countdown/Countdown", () => {
    ...
    it("props - days, hours, minutes, seconds", () => {
        ...
        node.setProps({ days: { visible: false }, hours: { visible: false }, minutes: { visible: false }, seconds: { visible: false } });
    ...
    it("props - onChange", (done) => {
        let node = TestUtils.mount({ value: 2000, onChange: () => { done(); } }, Countdown, props);
        node.instance().start();
        node.unmount();
    });
    it("props - onComplete", (done) => {
        let node = TestUtils.mount({ value: 1000, onComplete: () => { done(); } }, Countdown, props);
        node.instance().start();
        node.unmount();
    });
    ...
    it("__parseLong", () => {
        ...
        chai.assert.deepEqual(values, { days: 1, hours: 1, minutes: 1, seconds: 1 }, "Days, Hours, Minutes, Seconds must be 1s.");
